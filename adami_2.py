...
from pysph.tools.geometry import get_2d_block, get_2d_circle, remove_overlap_particles
...
from surface_tension import SummationDensity, SolidWallPressureBCnoDensity, ColorGradientAdami, ConstructStressMatrix, MomentumEquationPressureGradientAdami, MomentumEquationViscosityAdami, SurfaceForceAdami
...
class MultiPhase(Application):
    ...
    def create_particles(self):
        ...
        circle_x, circle_y = get_2d_circle(dx=dx, r=0.2, center=np.array([0.0, 0.0]))
        ...
        liquid = get_particle_array(
            ...
            rho=rho_circle, cs=cs_circle, additional_props=additional_props, constants=consts)
        liquid.nu[:] = nu1
        liquid.color[:] = 1.0
        ...
        gas.add_output_arrays(['V', 'color', 'cx', 'cy', 'nx', 'ny', 'ddelta',
                                  'kappa', 'N', 'scolor', 'p'])
        liquid.add_output_arrays(['V', 'color', 'cx', 'cy', 'nx', 'ny', 'ddelta',
                                 'kappa', 'N', 'scolor', 'p'])
        ...
        for i in range(len(gas.x)):
            ...
            gas.u[i] = v0*gas.x[i]*(1.0 - (gas.y[i]*gas.y[i])/(r0*R))*np.exp(-R/r0)/r0
            gas.v[i] = -v0*gas.y[i]*(1.0 - (gas.x[i]*gas.x[i])/(r0*R))*np.exp(-R/r0)/r0
        for i in range(len(liquid.x)):
            R = sqrt(r(liquid.x[i], liquid.y[i]) + 0.0001*liquid.h[i]*liquid.h[i])
    ...
    def create_equations(self):
        adami_equations = [
            ...
            Group(equations=[
                ...
                SolidWallPressureBCnoDensity(dest='wall', sources=['liquid', 'gas']),
            ]),
            Group(equations=[
                ColorGradientAdami(dest='liquid', sources=['liquid', 'wall', 'gas']),
                ColorGradientAdami(dest='gas', sources=[
                              'liquid', 'wall', 'gas']),
            ]),
            Group(equations=[ConstructStressMatrix(dest='liquid', sources=None, sigma=sigma, d=2),
                             ConstructStressMatrix(dest='gas', sources=None, sigma=sigma, d=2)]),
            Group(
                equations=[
